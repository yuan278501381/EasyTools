...
/** 文件名搜索的防抖时长分级：世界级 Search-As-You-Type 即时流式响应，对标 Everything 击键瞬出 */
export const NAME_DEBOUNCE_MS = {
  veryShort: 45, // <= 1 字符：单字母微缓冲，兼顾初筛与瞬间呈现
  short: 20,     // 2~3 字符：极速 20ms（1 刷新帧），边打字边出结果
  normal: 16,    // >= 4 字符：16ms 极致跟手，行云流水
} as const;
