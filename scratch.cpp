...
void initializeSubsystems(HWND hwnd) {
    ...
    tray.initialize(hwnd, easy::core::WinUtils::getAppDataDirectory() / L"assets\\icon.ico");

    tray.onSettings([]() { showSettingsWindow(); });
    ...
    easy::core::KeyboardHook::instance().setKeyDownCallback([](UINT vkCode) {
        easy::core::MessageBridge::instance().handleMessage(R"({"method":"core.keydown", "vkCode":)" + std::to_string(vkCode) + R"(})");
    });

    // 5. 加载并初始化插件
    auto& pm = easy::core::PluginManager::instance();
    auto pluginDir = easy::core::WinUtils::getAppDirectory() / L"plugins";
    pm.loadPlugins(pluginDir.string());
    pm.initializeAll(nullptr); // passing nullptr as ConfigManager if plugins handle their own config logic or expect PluginContext
...
void shutdownSubsystems() {
    easy::core::PluginManager::instance().shutdownAll();
    ...
    easy::tray::TrayIcon::instance().shutdown();
...
void showSettingsWindow() {
    ...
    config.title = L"EasyTools 设置";
    config.width = 900;
    config.height = 700;
    config.startUrl = L"http://localhost:5173";
    settingsWnd.show(config);
}

LRESULT CALLBACK MessageWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == easy::tray::TrayIcon::TRAY_CALLBACK_MESSAGE) {
